...
public class RedisCache<K, V> implements Cache<K, V> {
    ...
    private String getStringRedisKey(K key) {
        ...
        } else if(key instanceof SysUser){
            redisKey = ((SysUser)key).getNickName();
        }else if(key instanceof String){
            redisKey = key.toString();
        }else {
            redisKey = JSON.parseObject(JSON.toJSON(key).toString(), SysUser.class).getNickName();
        }
        return redisKey;
    }
